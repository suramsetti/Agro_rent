    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final columns = (constraints.maxWidth / desiredCardWidth)
              .floor()
              .clamp(2, 4);
...
void _showItemSheet(
  ...
  LeftoverItem item,
  List<LeftoverItem> all,
) {
  final similar = all
      .where(
        (i) =>
            i.id != item.id &&
            i.name.split(' ').first.toLowerCase() ==
                item.name.split(' ').first.toLowerCase(),
      )
      .take(6)
      .toList();
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const Spacer(),
                                Text(
                                  '${(s.discount * 100).toStringAsFixed(0)}% off',
                                  style: const TextStyle(color: Colors.green),
                                ),
                                Text(
...
void _showItemDeliveryDetails(BuildContext context, LeftoverItem item) {
  Navigator.push(
    context,
    MaterialPageRoute(builder: (_) => ItemDeliveryDetailsScreen(item: item)),
  );
}
